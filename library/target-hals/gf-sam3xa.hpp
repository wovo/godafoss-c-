...
// implementation of CPU clock speed switching
#include "target-hals/external/sam3xa.inc"
   ...
   pin_in_out_root
{
   ...
   //   godafoss::sam3xa< clock_frequency >::init();
   //}

   using resources = execute< godafoss::sam3xa< clock_frequency >::init__ >;
...
static void _adc_init_common(){
   ...
   godafoss::sam3xa< clock_frequency >::init__();
   ...
   be_timing_wait< waiting >
{
   ...
   //static void GODAFOSS_INLINE init(){
   //   sam3xa< clock_frequency >::init();
   //}

   using resources = execute< sam3xa< clock_frequency >::init__ >;
   ...
   static ticks_type GODAFOSS_INLINE now_ticks(){
      return sam3xa< clock_frequency >::now_ticks();
   }
